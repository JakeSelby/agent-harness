...
4. **Remove the worktree and its branch** with `harness worktree remove <name> --merged`: it
   clears the regenerable caches a gate run wrote, removes the checkout, then deletes the branch
   ...
   regenerable directory this repository writes that the built-in list misses.
5. **Audit** with `harness worktree audit`, and report every stale or dirty checkout it names
