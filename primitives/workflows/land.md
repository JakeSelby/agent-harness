...
2. **Record the merge,** from the worktree after a fetch: `harness intent merge --base origin/main`
   (the default branch) probes with `git merge-tree`, or pass `clean` or `conflicted` for a
   merge you ran. `harness usage --conflicts` reports it weekly. A conflict stops the workflow.
...
5. **Remove the worktree and its branch** with `harness worktree remove <name> --merged`: it
