...
- `harness worktree remove` finishes the cleanup after a squash merge. The quality gate has to run
  ...
  as dirty by `harness worktree audit`; removal now clears the regenerable caches it knows
  (`__pycache__`, `.pytest_cache`, `.mypy_cache`, `.ruff_cache`, `node_modules`, `.astro`, plus any
  `--also-clear NAME`) and audit reports a checkout holding only those as clean. Modified,
  ...
  `gh`, no merged pull request, or a tip the merged head does not match refuses with the reason
  and deletes nothing. `/land` now hands both steps to one command.
