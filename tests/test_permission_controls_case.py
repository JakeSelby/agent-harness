...
No test here launches a client. Three real Claude Code results recorded on 2026-09-22 stand in
for the turns, each with the transcript record carrying the mode the client reported for it and
the sentinel state it left behind (`fixtures/permission-controls/README.md` says how each was
produced). Nothing in this file decides an outcome: what a fixture turn did is read from the
recording, so a refused write classifies as a block, a declined turn as a decline and never as a
block, and the completed bypass turn as neither.

`bypass-declined.json` is the one synthetic element: the turn is real but its decline was induced
by a substitute prompt, because six live probe runs completed and none declined spontaneously.
"""
import json
import tempfile
import unittest
...
class Turn(object):
    """One recorded turn: the client's JSON result, its transcript record and what it left."""

    def __init__(self, name):
        self.name = name
        self.data = json.loads((FIXTURES / (name + ".json")).read_text())
        self.transcript = (FIXTURES / (name + ".transcript.jsonl")).read_text()
        self.wrote = RECORDED["turns"][name]["sentinel_present"]
...
class FakeHome:
    ...
    def session(self, prompt, tools=("Agent",), **kwargs):
        turn = self.turns.pop(0)
        self.calls.append((prompt, tuple(tools)))
        if turn.wrote:
            (self.project / MODULE.SENTINEL).write_text("")
        self.transcripts[turn.data["session_id"]] = turn.transcript
        return turn.data
...
def run_case(turns):
    ...
    with tempfile.TemporaryDirectory() as directory:
        return MODULE.case_permission_controls(FakeHome(directory, turns))


HAPPY = [DENIED, COMPLETED, DENIED]
...
class OutcomeTests(unittest.TestCase):
    ...
    def test_a_recorded_denial_is_a_block(self):
        self.assertFalse(DENIED.wrote)
        self.assertEqual(MODULE.turn_outcome(DENIED.wrote, DENIED.data), MODULE.BLOCKED)

    def test_a_recorded_decline_is_a_decline_and_never_a_block(self):
        self.assertFalse(DECLINED.wrote)
        self.assertEqual(MODULE.turn_outcome(DECLINED.wrote, DECLINED.data), MODULE.DECLINED)
        result, reason = MODULE.bypass_verdict(False, DECLINED.data, MODULE.BYPASS_MODE)
        self.assertEqual(result, "unverified")
        self.assertIn("declined", reason)

    def test_a_completed_bypass_turn_is_neither(self):
        self.assertTrue(COMPLETED.wrote)
        self.assertEqual(MODULE.turn_outcome(COMPLETED.wrote, COMPLETED.data), MODULE.COMPLETED)
        self.assertEqual(MODULE.bypass_verdict(True, COMPLETED.data, MODULE.BYPASS_MODE),
                         ("passed", ""))

    def test_each_recorded_turn_carries_the_mode_the_client_reported_for_it(self):
        with tempfile.TemporaryDirectory() as directory:
            home = FakeHome(directory, [DENIED, COMPLETED])
            manual = home.session(MODULE.SENTINEL_PROMPT, tools=())
            self.assertEqual(MODULE.turn_mode(home, manual), MODULE.MANUAL_MODE)
            bypass = home.session(MODULE.SENTINEL_PROMPT, tools=())
            self.assertEqual(MODULE.turn_mode(home, bypass), MODULE.BYPASS_MODE)

    def test_a_turn_the_client_reported_no_mode_for_says_so(self):
        with tempfile.TemporaryDirectory() as directory:
            home = FakeHome(directory, [])
            self.assertEqual(MODULE.turn_mode(home, {"session_id": "unread"}), "")
        self.assertIn("reported no permission mode", MODULE.mode_clause(""))
        self.assertIn(MODULE.BYPASS_MODE, MODULE.mode_clause(MODULE.BYPASS_MODE))
...
class PostureTests(unittest.TestCase):
    ...
    def test_the_case_walks_every_posture_and_reports_what_each_one_did(self):
        ...
        for needle in ("permissions=manual synced permission mode default and the client refused "
                       "the write, recording 1 permission denial(s)",
                       MODULE.ACK_KEY, "the acknowledged bypass synced " + MODULE.BYPASS_MODE,
                       "permissions=auto synced " + MODULE.AUTO_MODE,
                       "the client itself reported permission mode"):
            self.assertIn(needle, observation, msg=needle)

    def test_no_probe_turn_pre_approves_a_tool(self):
        with tempfile.TemporaryDirectory() as directory:
            home = FakeHome(directory, list(HAPPY))
            MODULE.case_permission_controls(home)
        self.assertEqual(home.calls, [(MODULE.SENTINEL_PROMPT, ())] * 3)
...
class VerdictTests(unittest.TestCase):
    ...
    def test_a_declined_bypass_turn_is_unverified_and_never_failed(self):
        outcome = self.probe([DENIED, DECLINED, DENIED])
    ...
    def test_a_blocked_bypass_turn_is_the_failure(self):
        outcome = self.probe([DENIED, DENIED, DENIED])
    ...
    def test_a_manual_posture_that_wrote_the_file_is_the_failure(self):
        outcome = self.probe([COMPLETED, COMPLETED, DENIED])
        self.assertEqual(outcome["result"], "failed")
    ...
    def test_a_manual_posture_the_model_declined_observed_no_restriction(self):
        outcome = self.probe([DECLINED, COMPLETED, DENIED])
        self.assertEqual(outcome["result"], "unverified")
        self.assertIn("declined", outcome["observation"])

    def test_an_unobserved_later_posture_keeps_the_earlier_readings(self):
        outcome = self.probe([DENIED, DECLINED, DENIED])
        self.assertIn("permissions=manual synced permission mode", outcome["observation"])
        self.assertIn(MODULE.ACK_KEY, outcome["observation"])

    def test_an_auto_posture_that_completed_is_reported_rather_than_judged(self):
        observation = run_case([DENIED, COMPLETED, COMPLETED])
