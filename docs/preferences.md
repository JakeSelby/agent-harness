...
the only variant that installs an output style, on either runtime; under `answer-card` and `off`
a sync installs none and takes back out the one a previous selection left, while an output style
you chose yourself is left exactly as it is.
