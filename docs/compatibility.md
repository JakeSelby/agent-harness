...
Claude Code and Codex are this release's integration targets. The v0.13.0 candidate floor
requires their CLIs on macOS and Linux, and that qualification has not yet been re-run against
this source. The v0.12.0 release carried no native qualification, so the v0.11.1 stable floor
remains the last one qualifying those CLIs on macOS and Linux. The Claude Code and Codex VS Code surfaces, Codex Desktop on macOS and the Claude Code
