...
costs; the turn line shows none of that. It is said
once per threshold and not once per turn — a session that stays above one is silent until it
reaches the next — and a resume reads the thresholds already said back out of the state file. A
context no response has reported yet is no crossing, so nothing is said rather than a size of
zero being invented. Like every other line here it is soft: nothing is blocked.
