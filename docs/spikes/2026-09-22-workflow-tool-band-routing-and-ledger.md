...
**`agentType` is a supported `agent()` option and it resolves the user's agent definitions.** The
third call ran as `reviewer` on that definition's model rather than the session's `sonnet`, in
session, with no isolated worker — the exact spawn `role_deny` refuses through the `Agent` tool.
...
1. **Guard the `Workflow` launch.** The harness cannot rewrite an `agent()` call inside a script, but
   ...
   `Agent` refusal carries. The tool input is the script itself, as `script` or `scriptPath`, so the
